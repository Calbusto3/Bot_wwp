...
class AutoRole(commands.Cog):
    ...
    async def setup_auto_role(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send(
            "Choisissez une condition parmi les suivantes :\n1. Nouveau membre\n2. Rejoindre un salon vocal\n3. Envoyer un message\n4. Réagir à un message",
            ephemeral=True
        )
        condition_message = await self.bot.wait_for(
            'message', check=lambda m: m.author == interaction.user and m.channel == interaction.channel
        )
        ...
        if role_condition not in self.config:
            self.config[role_condition] = {"role": role.name, "condition": condition}
            self.save_config()
    ...
    async def remove_auto_role(self, interaction: discord.Interaction):
        ...
        options = [discord.SelectOption(label=f"{data['role']} -> {data['condition']}", value=key) for key, data in self.config.items()]
        ...
        async def select_callback(interaction: discord.Interaction):
            ...
            if autorole:
                self.save_config()
                await interaction.response.edit_message(content=f"✅ L'auto-rôle `{autorole_key}` a été supprimé.", embed=None, view=None)
            else:
                await interaction.response.send_message("❌ Autorôle introuvable.", ephemeral=True)

        select.callback = select_callback

        embed = discord.Embed(title="Supprimer un autorôle", description="Sélectionne un autorôle à supprimer.", color=discord.Color.red())
        view = View(timeout=None)
        view.add_item(select)

        await interaction.response.send_message(embed=embed, view=view, ephemeral=True)

    async def show_auto_roles(self, interaction: discord.Interaction):
        ...
        options = [discord.SelectOption(label=f"{data['role']} -> {data['condition']}", value=key) for key, data in self.config.items()]
        select = Select(placeholder="Sélectionne un autorôle pour voir les détails", options=options)
        ...
        embed = discord.Embed(title="Afficher les autorôles", description="Sélectionne un autorôle pour afficher les détails.", color=discord.Color.blue())
        view = View(timeout=None)
        view.add_item(select)
